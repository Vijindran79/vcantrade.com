...
print(f"[MAGNIFY] Testing connection to {OLLAMA_IP}...\n")
...
for port in PORTS_TO_TEST:
    try:
        ...
        if result == 0:
            ...
            print(f"[FAIL] Port {port}: CLOSED/FILTERED")
        sock.close()
...
for port in [11434, 17197]:
    ...
    try:
        ...
        if response.status_code == 200:
            print(f"[OK] SUCCESS! Ollama is running on port {port}")
            models = response.json().get('models', [])
            print(f"[BOX] Available models: {[m['name'] for m in models]}")
            break
        else:
            print(f"[FAIL] HTTP {response.status_code} - Wrong port or service")
    except requests.exceptions.Timeout:
        print(f"[FAIL] TIMEOUT - Port is blocked by firewall")
    except requests.exceptions.ConnectionError:
        print(f"[FAIL] CONNECTION REFUSED - Nothing listening on this port")
    except Exception as e:
