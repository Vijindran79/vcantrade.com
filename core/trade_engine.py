...
from core.models import (
    LLMAnalysisOutput, MarketDataPoint, TradeRecord, 
    SafetyState, SignalAction, ConfidenceLevel
)
...
class TradeEngine:
    ...
    def process_signal(self, signal: LLMAnalysisOutput, mode: str = "TEACHER") -> Optional[TradeRecord]:
        """
        ...
        if mode == "TEACHER" or config.DRY_RUN:
            logger.info(f"[TEACHER MODE] Signal: {signal.action} {signal.asset} - {signal.reason}")
    ...
    def _check_safety(self) -> bool:
        ...
        if abs(self.safety_state.daily_pnl) >= config.MAX_DAILY_LOSS:
            logger.warning(f"Daily loss limit reached: ${self.safety_state.daily_pnl:.2f}")
        ...
        if len(self.open_trades) >= config.MAX_OPEN_POSITIONS:
            logger.warning(f"Max positions reached: {len(self.open_trades)}/{config.MAX_OPEN_POSITIONS}")
            return False
    ...
    def _execute_buy(self, signal: LLMAnalysisOutput) -> TradeRecord:
        ...
        logger.info(f"EXECUTED BUY: {trade.asset} @ {trade.entry_price} | SL: {trade.stop_loss} | TP: {trade.take_profit}")
        return trade
    ...
    def _execute_sell(self, signal: LLMAnalysisOutput) -> TradeRecord:
        ...
        logger.info(f"EXECUTED SELL: {trade.asset} @ {trade.entry_price} | SL: {trade.stop_loss} | TP: {trade.take_profit}")
        return trade
    ...
    def _execute_close(self, signal: LLMAnalysisOutput) -> Optional[TradeRecord]:
        ...
        for i, trade in enumerate(self.open_trades):
            if trade.asset == signal.asset:
                ...
                trade.pnl = (trade.exit_price - trade.entry_price) if trade.action == SignalAction.BUY else (trade.entry_price - trade.exit_price)
                ...
                if trade.pnl and trade.pnl < 0 and trade.stop_loss:
                    self.cooldown_until = datetime.utcnow() + timedelta(seconds=config.COOLDOWN_AFTER_STOP)
                    logger.warning(f"Stop loss hit - cooldown for {config.COOLDOWN_AFTER_STOP}s")
    ...
    def _save_trade(self, trade: TradeRecord):
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            trade.id, trade.timestamp.isoformat(), trade.asset, trade.action.value,
            trade.entry_price, trade.stop_loss, trade.take_profit, trade.exit_price,
            trade.pnl, trade.confidence.value, trade.ai_reason, trade.mode, trade.status,
    ...
    def _update_trade(self, trade: TradeRecord):
        ...
        """, (trade.exit_price, trade.pnl, trade.status, 
              trade.closed_at.isoformat() if trade.closed_at else None, trade.id))
        self.conn.commit()
