...
class CalibrationWizardDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        # Title
        title = QLabel("[MOUSE] RPA Coordinate Calibration Wizard")
        ...
        self._capture_btn = QPushButton("[PIN] Click & Capture Coordinates")
    ...
    def _on_capture(self):
        ...
        try:
            ...
            self._cal.save()
            self._status.setText(f"[OK] Captured ({int(x)}, {int(y)}) for '{POINT_LABELS.get(point, point)}'")
            self._status.setStyleSheet(f"color: {GREEN};")
            logger.info("Calibration point '%s' captured at (%d, %d)", point, x, y)
        except Exception as exc:
            self._status.setText(f"[FAIL] Could not capture: {exc}")
